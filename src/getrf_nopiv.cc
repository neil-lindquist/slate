    ...
    Options const& opts )
{
    ...
    const int queue_0 = 0; // tr left-panel trsm, la left-panel gemm
    const int queue_1 = 1; // tr top-panel  trsm, la top-panel gemm
    const int queue_2 = 2; // trailing gemm
    const int queue_3 = 3; // la left-panel trsm, diagonal la gemm
    const int queue_4 = 4; // la top-panel  trsm
    ...
    if (target == Target::Devices) {
        // queues 3 and 4 only used by the lookahead
        // batch array size will be set as needed
        A.allocateBatchArrays(0, lookahead > 0 ? 5 : 3);
    ...
    int64_t max_mt_nt = std::max(A.mt(), A.nt());
    bool is_shared = lookahead > 0;

    // OpenMP needs pointer types, but vectors are exception safe
    std::vector< uint8_t > Axx_vector(9*max_mt_nt);
    uint8_t* A11 = Axx_vector.data();
    uint8_t* A12 = A11 + max_mt_nt;
    uint8_t* A13 = A12 + max_mt_nt;
    uint8_t* A21 = A13 + max_mt_nt;
    uint8_t* A22 = A21 + max_mt_nt;
    uint8_t* A23 = A22 + max_mt_nt;
    uint8_t* A31 = A23 + max_mt_nt;
    uint8_t* A32 = A31 + max_mt_nt;
    uint8_t* A33 = A32 + max_mt_nt;
    ...
    #pragma omp master
    {
        for (int64_t k = 0; k < min_mt_nt; ++k) {
            ...
            // whether to create the corresponding tasks
